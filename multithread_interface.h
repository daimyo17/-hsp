...
typedef TASK (*TSK_ADR)(void);

#define HANDLER void interrupt
typedef HANDLER (*HANDLER_ADR)(void);
typedef struct{
...
void _init(void);
void create(TSK_IDENT *tskidadr, TSK_ADR runadr, uschar prior, usshort stksz, void *dadr);
#define _create(tskid, runadr, prior, stksz, data) create(&tskid, runadr, prior, stksz, &data)
void _destroy(TSK_IDENT tskid);
void _activate(TSK_IDENT tskid);
void _disactivate(TSK_IDENT tskid);
void _sleep(usint timeout);
void _wakeup(TSK_IDENT tskid);
